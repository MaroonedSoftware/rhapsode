...
/**
 * What the API section's groups are called. ContractKit names a group after its contract file's
 * `area` and title-cases it, which makes `openai` "Openai" and the shared types "Rhapsode". Renaming
 * an area would move the generated SDK and Python modules with it, so the words are fixed here, in
 * the one place that only draws them.
 */
...
type SidebarItems = Awaited<ReturnType<SidebarItemsGenerator>>;
type SidebarItemsGenerator = NonNullable<PluginOptions['sidebarItemsGenerator']>;

/** Relabels every category the autogenerated API sidebar holds, at any depth. */
function relabel(items: SidebarItems): SidebarItems {
    return items.map(item =>
        item.type === 'category' ? { ...item, label: AREA_LABELS[item.label] ?? item.label, items: relabel(item.items) } : item,
    );
}
        ...
        [
            'classic',
            {
                docs: {
                    ...
                    sidebarItemsGenerator: async ({ defaultSidebarItemsGenerator, ...args }) => relabel(await defaultSidebarItemsGenerator(args)),
                },
                blog: false,
