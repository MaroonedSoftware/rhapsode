...
/**
 * The Python modules `pnpm python:sync` installs into the shared dev venv. Chatterbox is here
 * without torch: its adapter imports its engine lazily, so the module loads and its tests stub the
 * model.
 */
export const DEV_MODULES = ['rhapsode_worker', 'rhapsode_engine_tone', 'rhapsode_conform', 'rhapsode_engine_chatterbox'];
