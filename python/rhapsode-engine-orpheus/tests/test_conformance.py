...
Everything under test is the real adapter and the real SDK, served over a socket. Only llama.cpp,
SNAC, torch and the hub are fakes. No stub can say whether `<laugh>` is performed; what this says is
    ...
    process = subprocess.Popen(
        ...
        env={
            **os.environ,
            ...
            "RHAPSODE_WORKER_ENGINE": "orpheus",
...
def test_every_cue_it_claims_changes_the_audio_and_clear_throat_is_not_claimed(stubbed: str) -> None:
    with Worker(stubbed) as worker:
        report = run(worker)
        _, capabilities = worker.get("/capabilities")

    cue_checks = [result for result in report.results if result.name.startswith("cue ")]
    assert len(cue_checks) == 7 * 2
    assert all(result.passed for result in cue_checks)
    for variant in capabilities["variants"].values():
